/*
    The 'front end/ JavaScript bot comprises five main functions:
    main()    ---- starts the bot and orchestrates the other finctions.
    checkPrice()    ---- responds to swap events to see if a profitable trade opportunity exists
    determineDirection()    ---- determines which exchange to buy the 'arb against' token (e.g. 'Shib'and sell the 'arb against' (Ship) tokens
    determineProfitability()   -- determines profitability after 2 x trading fees and gas fees)
    executeTrade()    ---- Executes a profitable trade on the blockchain using Arbitrage.sol

    -- The main() function monitors swap events occuring on both Uniswap and Sushiswap.
*/
...
const fs = require('fs')  // VOH added 13/03/2022 to log timestamps and price differences
...
const main = async () => {
    ...
    console.log(`Entered main (occurs once per session)\n`)  // VOH added 13/03/2022
    // add writefile here
    console.log(`Getting uPair and sPair Contracts-------------------\n`) // VOH added 13/03/2022
    ...
    // VOH Added Comment - Listening for swap event on Uniswap
    ...
    // VOH Added - Listening for swap event on Sushiswap
    ...
    console.log("Main Function:  Waiting for swap event...")
}
...
const checkPrice = async (exchange, token0, token1) => {
    ...
    console.log('\u0007'); // VOH added 'Bell' sound 05/03/2022
    var now = new Date();  // VOH added 'Date/Time' 06/032022
    console.log(now.toUTCString());  // VOH added 6/3/2022
    console.log(`Percentage Difference: ${priceDifference}%\n`)
    // VOH added write to log file
    const path = './botlog.txt'
    
    // fs.appendFile(path, priceDifference,err => {   - Works
    fs.appendFile(path, priceDifference + '\n',err => {
        if (err) {
            console.error(err)
            return
        }
        // Done!
        // Search for both positive and negative price differences specified price difference 
        if (priceDifference >= 0 && priceDifference > difference  || priceDifference <= 0 && priceDifference <= difference) {
            var rightNow = new Date();  // VOH added 'Date/Time' 06/032022
            var rightNowStamp = rightNow.toUTCString();  // VOH added 6/3/2022
            fs.appendFile(path, rightNowStamp + ' - ' + 'Suitable Trade @ Price difference of ' + priceDifference + ' %' + '\n',err =>{
                if (err) {
                    console.error(err)
                    return
                }
